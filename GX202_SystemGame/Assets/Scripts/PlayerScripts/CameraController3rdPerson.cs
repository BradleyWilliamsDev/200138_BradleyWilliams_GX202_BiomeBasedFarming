    ...
    void FixedUpdate () {
        ...
        if (InputController.moveVector.y < 0) {
            cameraTransform.localRotation = Quaternion.Lerp (cameraTransform.localRotation,
                Quaternion.Euler (cameraTiltBack * Mathf.Abs (InputController.moveVector.y)),
        ...
        if (InputController.moveVector.x != 0) {
            gripTransform.localRotation = Quaternion.Lerp (gripTransform.localRotation,
                (InputController.moveVector.x > 0 ?
                    Quaternion.Euler (cameraTiltSide * Mathf.Abs (InputController.moveVector.x)) :
                    Quaternion.Euler (cameraTiltSideLeft * Mathf.Abs (InputController.moveVector.x))),
                Time.fixedDeltaTime * 1f);
